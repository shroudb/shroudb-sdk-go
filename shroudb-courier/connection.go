...
//
// This file is an implementation detail of the ShroudbCourier client library.
...
func dial(host string, port int, useTLS bool) (*connection, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("shroudb_courier: connect %s: %w", addr, err)
	}
...
func (c *connection) execute(args ...string) (any, error) {
	...
	if _, err := io.WriteString(c.conn, buf); err != nil {
		return nil, fmt.Errorf("shroudb_courier: write: %w", err)
	}
	return c.readFrame()
}
...
func (c *connection) readFrame() (any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("shroudb_courier: read: %w", err)
	}
	if len(line) < 3 {
		return nil, fmt.Errorf("shroudb_courier: short response")
	}
	tag := line[0]
	...
	case ':':
		...
		if err != nil {
			return nil, fmt.Errorf("shroudb_courier: invalid integer: %s", payload)
		}
		return n, nil
	case '$':
		length, err := strconv.Atoi(payload)
		if err != nil {
			return nil, fmt.Errorf("shroudb_courier: invalid bulk length: %s", payload)
		}
		if length < 0 {
			return nil, nil
		}
		...
		if _, err := io.ReadFull(c.reader, data); err != nil {
			return nil, fmt.Errorf("shroudb_courier: bulk read: %w", err)
		}
		return string(data[:length]), nil
	case '*':
		count, err := strconv.Atoi(payload)
		if err != nil {
			return nil, fmt.Errorf("shroudb_courier: invalid array length: %s", payload)
		}
		arr := make([]any, count)
		for i := range count {
	...
	case '%':
		count, err := strconv.Atoi(payload)
		if err != nil {
			return nil, fmt.Errorf("shroudb_courier: invalid map length: %s", payload)
		}
	...
	case '_':
		return nil, nil
	default:
		return nil, fmt.Errorf("shroudb_courier: unknown response type: %c", tag)
	}
}
